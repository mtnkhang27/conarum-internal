...
type PickTone = 'neutral' | 'correct' | 'incorrect';
...
function resolvePickTone(scoreBet: ScoreBetRow): PickTone {
  if (scoreBet.isCorrect === true) return 'correct';
  if (scoreBet.isCorrect === false) return 'incorrect';
  return 'neutral';
}

function renderScorePick(scoreBet: ScoreBetRow, hasResult: boolean) {
  const tone = hasResult ? resolvePickTone(scoreBet) : 'neutral';

  return (
    <div
      key={scoreBet.betId || `${scoreBet.predictedHomeScore}-${scoreBet.predictedAwayScore}`}
      className="inline-flex items-center gap-1.5"
    >
      <ScorePickBox
        value={typeof scoreBet.predictedHomeScore === 'number' ? String(scoreBet.predictedHomeScore) : '-'}
        tone={tone}
      />
      <span className="text-[11px] font-semibold text-muted-foreground">:</span>
      <ScorePickBox
        value={typeof scoreBet.predictedAwayScore === 'number' ? String(scoreBet.predictedAwayScore) : '-'}
        tone={tone}
      />
    </div>
  );
...
function resolveOutcomeTone(pick?: string | null, homeScore?: number | null, awayScore?: number | null): PickTone {
  ...
  return actualOutcome === pick ? 'correct' : 'incorrect';
}
...
export function RecentPredictionsCard({ tournamentId, className }: RecentPredictionsCardProps) {
  ...
  if (isLoading) {
    return (
      <div className={containerClassName}>
        <div className={initialStateClassName}>
  ...
  if (error) {
    return (
      <div className={containerClassName}>
        <div className={cn(initialStateClassName, 'p-6 text-sm text-destructive')}>
        ...
        <Table className="w-full min-w-[980px] table-auto text-[12px]">
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              <TableHead className="sticky top-0 z-10 min-w-[130px] bg-card px-3 py-2.5">
              ...
              <TableHead className="sticky top-0 z-10 min-w-[160px] bg-card px-3 py-2.5 text-center">
              ...
              <TableHead className="sticky top-0 z-10 min-w-[110px] bg-card px-3 py-2.5 text-center">
                {t('predictionDashboard.columns.result', 'Result')}
              </TableHead>
              <TableHead className="sticky top-0 z-10 min-w-[150px] bg-card px-3 py-2.5 text-center">
                {t('predictionDashboard.columns.scorePickEarned', 'Score Pick Earned')}
              </TableHead>
          ...
          <TableBody>
            {data.map((item) => {
              const outcomeTone = resolveOutcomeTone(item.pick, item.homeScore, item.awayScore);
              ...
              const fallbackCorrectCount = visibleScoreBets.filter(
                (scoreBet) => scoreBet.isCorrect === true
              ).length;
              const resolvedCorrectCount = correctScoreBetCount ?? fallbackCorrectCount;
              const configuredScoreBetCount =
                scoreBetMaxBets !== null
                  ? Math.max(0, Math.trunc(scoreBetMaxBets))
                  : visibleScoreBets.length;
              const scoreBetSummary =
                scoreBetPrizeAmount !== null
                  ...
                  <TableCell className="px-3 py-2.5 align-top whitespace-normal">
                    <div className="flex flex-col gap-1.5">
                      <div className="flex items-center gap-1.5">
                        <TeamFlag
                          code={item.homeFlag}
                          crest={item.homeCrest}
                          name={item.homeTeam || 'TBD'}
                        />
                      ...
                      <div className="flex items-center gap-1.5">
                        <TeamFlag
                          code={item.awayFlag}
                          crest={item.awayCrest}
                          name={item.awayTeam || 'TBD'}
                        />
                        <span className="text-[12px] font-medium text-foreground">
                          {item.awayTeam || 'TBD'}
                        </span>
