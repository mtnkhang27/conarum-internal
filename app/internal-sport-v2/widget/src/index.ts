/**
 * CLAIR2 Chat Widget — Web Component Entry Point
 *
 * This file:
 * 1. Defines a custom element <clair2-chat> with Shadow DOM
 * 2. Injects styles into the shadow root
 * 3. Renders the React ChatWidget inside the shadow
 * 4. Exposes window.CLAIR2Chat global API
 *
 * Usage:
 *   <script src="https://clair2.../widget/clair2-chat.js"></script>
 *   <script>
 *     CLAIR2Chat.init({
 *       apiUrl: 'https://clair2.../api/v1',
 *       token: 'Bearer ...',
 *       documentId: 'a1b2c3d4...',
 *     });
 *   </script>
...
let currentConfig: CLAIR2ChatConfig | null = null;
let isOpen = false;
...
// ── Public API ──
function init(config: CLAIR2ChatConfig) {
    // Validate required fields
    if (!config.apiUrl) throw new Error('[CLAIR2Chat] apiUrl is required');
    if (!config.token) throw new Error('[CLAIR2Chat] token is required');
    if (!config.documentId) throw new Error('[CLAIR2Chat] documentId is required');
    ...
    hostElement.id = 'clair2-chat-widget';
    ...
    if (config.theme) {
        ...
        if (config.theme.primaryColor) {
            overrides += `--clair2-primary: ${config.theme.primaryColor};`;
        }
        if (config.theme.primaryHoverColor) {
            overrides += `--clair2-primary-hover: ${config.theme.primaryHoverColor};`;
        }
        if (config.theme.fontFamily) {
            overrides += `--clair2-font: ${config.theme.fontFamily};`;
        }
        overrides += '}';
        styleEl.textContent += '\n' + overrides;
    }
...
function emit(event: string, ...args: any[]) {
    eventListeners.get(event)?.forEach(cb => {
        try { cb(...args); } catch (e) { console.error('[CLAIR2Chat] Event error:', e); }
    });
}
...
// Assign to window
(window as any).CLAIR2Chat = CLAIR2Chat;

export default CLAIR2Chat;
